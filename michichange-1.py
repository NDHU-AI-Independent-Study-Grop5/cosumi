...
def empty_position():
    ...
    return Position(board=empty, cap=(0, 0), n=0, ko=None, last=None, last2=None, komi=0.0)
...
def print_pos(pos, f=sys.stderr, owner_map=None,endflag = None):
    ...
    #try:
        ...
        #print("%1d"%(sn))
        #print("%c"%board[sn])
    pretty_board = ' '.join(board.rstrip()) + ' '#rstrip刪除字串末端的自，莫認為空格
    ...
    #print("\n".join(pretty_board), file=f)
    #print ('%1d'%(pretty_ownermap.count('x')),file=f)
    if owner_map is not None:
        pretty_ownermap = ''
        for c in range(W*W):
            if board[c].isspace():
...
def parse_coord(s):
    if s == 'pass':
        return None
    #print("%d\n"%(W+1 + (N - int(s[1:])) * W + colstr.index(s[0].upper())))
    return W+1 + (N - int(s[1:])) * W + colstr.index(s[0].upper())
    
def parse_coord_fin(s):
    if s == 'pass':
        return -1
    #print("%d\n"%(W+1 + (N - int(s[1:])) * W + colstr.index(s[0].upper())))
    return W+1 + (N - int(s[1:])) * W + colstr.index(s[0].upper())
...
def countingB(board,count):
    for c in range(W*W):
        if board[c] == '.':
            if board[c+1] == 'X' or board[c-1] == 'X' or board[c+W] == 'X' or board[c-W] == 'X':
                count = count + 1
    return count
    
def countingW(board,count):
    for c in range(W*W):
        if board[c] == '.':
            if board[c+1] == 'O' or board[c-1] == 'O' or board[c+W] == 'O' or board[c-W] == 'O':
                count = count + 1
    return count
    
    
def print_pos_fin(board,c,f=sys.stderr):
    c_last = c
    pretty_board = ' '.join(board.rstrip()) + ' '
    #print("\n".join(pretty_board), file=f)
    if c_last != -1:
        pretty_board = pretty_board[:c_last*2-1] + '(' + board[c_last] + ')' + pretty_board[c_last*2+2:]
    rowcounter = count()
    pretty_board = [' %-02d%s' % (N-i, row[2:]) for row, i in zip(pretty_board.split("\n")[1:], rowcounter)]
    print("\n".join(pretty_board), file=f)
    print('    ' + ' '.join(colstr[:N]), file=f)
...
def print_score(board,ff=sys.stderr):
    print("black is %d"%board.count('X'))
    print("white is %d"%board.count('O'))
...
def take_over(pos, stdout, owner_map,tree,f=sys.stderr):
    tree.pos = pos
    sys.stdout = stdout
    ...
    for c in range(W*W):
        if board[c] == 'x':
            board = board[:c] + 'O' + board[c+1:]
            
    countB = countingB(board,countB)
    countW = countingW(board,countW)
    #print("$$%d"%countW)
    if countW > 2:
        for c in range(W*W):
            if board[c] == '.':
    ...
    #print("$$%d"%countW)
    while True:
        print_pos_fin(board,c_lastcom)
        c_lastcom = -1
        c_lastply = -1
        c_lasts = raw_input("Your move: ")
        try:
            c_lastply = parse_coord_fin(c_lasts)
        except:
            print('An incorrect move')
            continue
        if board[c_lastply] != '.' and c_lastply != -1:
            print("Bad move (not empty point)")
            continue
        if countB > 2:
            if board[c_lastply] == '.':
                board = board[:c_lastply] + 'X' + board[c_lastply+1:]
                countB = countB - 1
                print_pos_fin(board,c_lastply)
        else:
            if c_lastply != -1:
                print("Bad choise,you will be eat by computer!!!")
        if countW > 2:
            for c in range(W*W):
                if board[c] == '.':
                    board = board[:c] + 'O' + board[c+1:]
                    ...
                    #print("$$%d"%countW)
                    break
        if c_lastcom == -1 and c_lastply == -1:
            print_score(board)
            break
    #print('##%d'%countB)
    #print('##%d'%countW)
...
def count_false(pos, f=sys.stderr, owner_map=None,endflag = None):
    if pos.n % 2 == 0:  # to-play is black
        board = pos.board.replace('x', 'O')
        Xcap, Ocap = pos.cap
    else:  # to-play is white
    ...
    count_false_flag = 0
    int(count_false_flag)
    for c in range(W*W):
        if board[c] == 'O':
            count_false_flag = count_false_flag + 1
    return count_false_flag
    
def computer():

    sessions_number = int(raw_input("how many sessions you need: "))
    b_win_sessions = 0.0
    for i in range(0,sessions_number):
        tree = TreeNode(pos=empty_position())
        tree.expand()
        owner_map = W*W*[0]
        print('now_sessions_number:%d' % (i+1,))
        while True:
            ...
            if tree.pos.last is None:
                ...
                if tree.pos.n % 2:
                    score = -score
                if(score > 0):
                    b_win_sessions = b_win_sessions + 1
                print('Game over, score: B%+.1f' % (score,))
                break
    float(sessions_number)
    print('%d %d' % (b_win_sessions,sessions_number))
    
    b_win_rate = 0.0
    b_win_rate = (b_win_sessions/sessions_number)*100
    print('b_win_rate:%d percent' % (b_win_rate,))
...
def game_io(computer_black=False):
    ...
    while True:
        ...
        print("\n\n+++++++++++++++++++")
        if(flagc == 0):
            io = int(raw_input("enter '\"'1'\"' for computer-play: "))
            if(io == 1):
                computer()
                flagc = 1
        if(flagc == 1):
        ...
        if not (tree.pos.n == 0 and computer_black):
            ...
            print_pos(tree.pos)#只print board
        
        owner_map = W*W*[0]
        tree = tree_search(tree, N_SIMS, owner_map)
        if tree.pos.last is None:
            flag = count_false(tree.pos, sys.stdout, owner_map)
            if flag <= 6 and flag > 0:
                print("computer pass")
                continue
            take_over(tree.pos, sys.stdout, owner_map,tree)
            break
